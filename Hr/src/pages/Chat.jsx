...
const Chat = () => {
  ...
  return (
    <div className="flex h-screen bg-[#0C1014] text-gray-100">
      {/* --- SIDEBAR --- */}
      <div className={`${chatListOpen ? 'block' : 'hidden'} md:flex flex-col w-full md:w-80 border-r border-[#1F2429] h-full`}>
        {/* Header */}
        <div className="p-5 border-b border-[#1F2429] flex items-center justify-between shrink-0">
        ...
        <div className="px-5 py-4 shrink-0">
          <div className="relative group">
            ...
            <input
              ...
              className="w-full bg-[#1F2429] border border-[#1F2429] rounded-xl pl-10 pr-4 py-2.5 text-sm outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all font-medium text-gray-200 placeholder-gray-500"
            />
          </div>
        </div>
            ...
            <>
              {displayedContacts.length === 0 && (
                <div className="flex flex-col items-center justify-center h-40 text-center">
                  <div className="w-12 h-12 bg-[#1F2429] rounded-full flex items-center justify-center mb-3">
              ...
              {displayedContacts.map(contact => (
                <div
                  ...
                  className={`group flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-all duration-200 mb-1 ${selectedChat === contact.id ? 'bg-[#1F2429] border border-[#1F2429]' : 'hover:bg-[#1F2429] border border-transparent'}`}
                >
                  {/* Avatar + Status */}
                  <div className="relative shrink-0">
                    <div className="w-12 h-12 rounded-full bg-linear-to-b from-gray-700 to-gray-800 border border-gray-600 flex items-center justify-center text-gray-300 font-bold text-sm shadow-sm">
                      {contact.avatar}
                    </div>
                    {/* Status Dot */}
                    <div className={`absolute bottom-0.5 right-0.5 w-3.5 h-3.5 rounded-full border-2 border-[#0C1014] ${contact.status === 'online' ? 'bg-green-500' : 'bg-gray-500'}`}></div>
                  </div>

                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-0.5">
                      <h3 className={`text-sm truncate ${contact.unread ? 'font-bold text-white' : 'font-semibold text-gray-300'}`}>
                        {contact.name}
                      </h3>
                      {contact.time && <span className="text-[10px] text-gray-500 font-medium shrink-0 ml-2">{contact.time}</span>}
                    </div>
                    <div className="flex items-center justify-between">
                      <p className={`text-xs truncate max-w-40 ${contact.unread ? 'text-white font-medium' : 'text-gray-500'}`}>
      ...
      {/* --- MESSAGING AREA --- */}
      <div className={`${chatListOpen ? 'hidden md:flex' : 'flex'} flex-1 flex-col bg-[#0C1014] relative`}>
        {selectedChat ? (
          <>
            {/* Chat Header */}
            <div className="h-[76px] bg-[#0C1014] border-b border-[#1F2429] flex items-center justify-between px-6 sticky top-0 z-10 shadow-xs">
              <div className="flex items-center gap-4">
                ...
                <div className="relative">
                  ...
                  <div className={`absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-[#0C1014] ${activeChatUser?.status === 'online' ? 'bg-green-500' : 'bg-gray-500'}`}></div>
                </div>
                <div>
              ...
              <div className="flex items-center gap-4 text-gray-500">
                <button className="hover:text-blue-500 transition-colors p-2 rounded-full hover:bg-[#1F2429]"><Phone size={20} /></button>
                <button className="hover:text-blue-500 transition-colors p-2 rounded-full hover:bg-[#1F2429]"><Video size={20} /></button>
                <button className="hover:text-gray-300 transition-colors p-2 rounded-full hover:bg-[#1F2429]"><MoreVertical size={20} /></button>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              {conversation.map(msg => (
                <div key={msg.id} className={`flex ${msg.sender === "me" ? "justify-end" : "justify-start"} group`}>
                  <div className={`relative max-w-[70%] md:max-w-[60%] rounded-2xl px-5 py-3 shadow-sm ${msg.sender === "me" ? "bg-blue-600 text-white rounded-tr-sm" : "bg-[#262626] text-white border border-[#262626] rounded-tl-sm"}`}>
                    ...
                    {/* Action Menu (Hover) */}
                    {msg.sender === "me" && !msg.is_deleted && (
                      <div className="absolute top-2 right-full mr-2 opacity-0 group-hover:opacity-100 transition-opacity flex gap-1 bg-[#262626] shadow-md rounded-lg p-1 border border-[#1F2429]">
                        <button onClick={() => {
                          const newText = prompt("Edit message:", msg.text);
                          if (newText && newText !== msg.text) handleEditMessage(msg.id, newText, selectedChat);
                        }} className="p-1.5 hover:bg-[#1F2429] rounded text-gray-400" title="Edit">
                          <Edit2 size={14} />
                        </button>
                        <button onClick={() => handleDeleteMessage(msg.id, selectedChat)} className="p-1.5 hover:bg-[#1F2429] text-red-500 rounded" title="Delete">
                          <Trash2 size={14} />
                        </button>
                      </div>
                    )}
            ...
            {/* Input Area */}
            <div className="p-4 bg-[#0C1014] border-t border-[#1F2429]">
              <div className="flex items-center gap-3 max-w-4xl mx-auto bg-[#1F2429] border border-[#1F2429] rounded-xl p-2 pr-2 shadow-xs focus-within:ring-1 focus-within:ring-blue-500 focus-within:border-blue-500 transition-all">
                <input
                  type="text"
                  value={inputMessage}
                  ...
                  className="flex-1 bg-transparent px-3 py-2 text-sm text-white outline-none placeholder-gray-500"
                />
                <button
                  onClick={handleSendMessage}
                  disabled={!inputMessage.trim()}
                  className={`p-2.5 rounded-lg transition-all ${inputMessage.trim() ? 'bg-blue-600 text-white shadow-md hover:bg-blue-700 transform hover:scale-105' : 'bg-[#2A3138] text-gray-500 cursor-not-allowed'}`}
                >
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-gray-500">
            <div className="w-24 h-24 bg-[#1F2429] rounded-full flex items-center justify-center mb-6">
              <Info size={40} className="text-gray-600" />
            </div>
            <p className="text-lg font-semibold text-gray-300">No Chat Selected</p>
